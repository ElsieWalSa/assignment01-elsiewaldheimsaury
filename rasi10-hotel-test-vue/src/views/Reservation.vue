...
export default class ReservationComponent extends Vue {
  ...
  private valiate() {
    ...
    if (
      ...
      !this.clients.find((c) => c.id === this.reservation.client)
    ) {
    ...
    if (
      ...
      !this.rooms.find((c) => c.id === this.reservation.room)
    ) {
    ...
    if (
      ...
      !this.bills.find((c) => c.id === this.reservation.bill)
    ) {
  ...
  public async save() {
    ...
    if (this.mode === "create") {
      ...
      this.reservation = await editReservation(
        this.reservation.id,
        this.reservation,
      );
    }
...
<template>
  <div class="container">
    <h2>
      ...
      <a v-if="mode === 'edit'" class="btn red" @click="remove">{{
        deleting ? "..." : "Delete"
      }}</a>
    </h2>
