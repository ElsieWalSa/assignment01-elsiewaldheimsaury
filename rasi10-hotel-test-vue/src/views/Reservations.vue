...
export default class Reservations extends Vue {
  ...
  public client(reservation) {
    return (
      this.clients.find((client) => client.id === reservation.client) || {
        name: "No client",
      }
    );
  }
...
<template>
  <div class="container">
    <h2>
      ...
      <router-link class="btn blue" to="/reservation/new"
    ...
    <div v-if="!loading" class="reservations">
      ...
      >
        <h3>
          ...
          {{ reservation.start || "No start" }} -
          {{ reservation.end || "No end" }}
        </h3>
        ...
        <div class="client">
          Client: {{ reservation.client || "No client" }}
        </div>
